...
To use it, place the `<owadview/>` tag (no attributes required) inside of a `<div>` tag, with the size of the standard IAB ad unit you wish to display.
...
While this article was originally aimed at Overwolf platform apps, the general ad details are also relevant for Overwolf Electron apps as well.
